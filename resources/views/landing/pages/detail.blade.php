            ...
            <div class="row">
                <div class="col-md-9 col-sm-9 col-xs-12">
                    <div class="property_single_details_slide"
                        style="width: 100%; height: 500px; overflow: hidden; border-radius: 8px;">
                    ...
                    <div class="property_single_details_price">
                        ...
                        <h4>
                            @if($itemType === 'attraction')
                                Rp. {{ number_format((float)$item->ticket_price, 0, ',', '.') }} /Pax
                            @else
                                Rp. {{ $item->price_range }} /Pax
                            @endif
                        </h4>
                    </div>
                                    ...
                                    @if ($approvedReviews->count() > 0)
                                        @foreach ($approvedReviews as $review)
                                            <div class="review-item"
                                                ...
                                                <small
                                                    style="color: #999;">{{ $review->created_at->format('d M Y') }}</small>
                                            </div>
                                        @endforeach
                                    @else
                    ...
                    <div class="single_property_form">
                        ...
                        <form class="form" method="POST" action="{{ route('reviews.store') }}">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-12">
                                    <input type="email" name="visitor_email" class="form-control" id="email"
                                        placeholder="Email" required="required" value="{{ old('visitor_email') }}">
                                </div>
                                <div class="form-group col-md-12">
                                    <label>Rating</label>
                                    <div class="rating-input"
                                        style="display: flex; gap: 8px; font-size: 28px; margin: 10px 0;">
                                        @for ($i = 1; $i <= 5; $i++)
                                            ...
                                            <label for="star{{ $i }}" title="{{ $i }} star(s)"
                                                ...
                                                onmouseover="ratingHover({{ $i }})" onmouseout="ratingOut()">
                                                <i class="fa fa-star"></i>
                                            </label>
                                        @endfor
                                    </div>
                                ...
                                <div class="form-group col-md-12 mbnone">
                                    <textarea rows="6" name="comment" class="form-control" id="description" placeholder="Your Review"
